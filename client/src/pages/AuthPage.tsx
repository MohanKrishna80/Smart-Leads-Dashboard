      ...
      <div className="mx-auto grid min-h-screen max-w-6xl items-center gap-10 px-5 py-10 lg:grid-cols-[1fr_440px]">
        <section>
          ...
          <p className="mt-4 max-w-2xl text-lg leading-8 text-slate-600 dark:text-slate-300">
            A focused workspace for sales teams to capture leads, track qualification, filter opportunities, and move quickly from inquiry to action.
          </p>
          <div className="mt-8 grid max-w-2xl gap-3 sm:grid-cols-3">
            {["JWT Auth", "RBAC", "CSV Export"].map((item) => (
              <div key={item} className="rounded-lg border border-slate-200 bg-white p-4 text-sm font-bold dark:border-slate-800 dark:bg-slate-900 dark:text-white">
                {item}
              </div>
            ))}
          </div>
        ...
        <section className="rounded-lg bg-white p-6 shadow-panel dark:bg-slate-900">
          <div className="mb-6 flex rounded-md bg-slate-100 p-1 dark:bg-slate-800">
            <button
              type="button"
              className={`h-10 flex-1 rounded-md text-sm font-bold ${mode === "login" ? "bg-white shadow-sm dark:bg-slate-950 dark:text-white" : "text-slate-500"}`}
            ...
            <button
              type="button"
              className={`h-10 flex-1 rounded-md text-sm font-bold ${mode === "register" ? "bg-white shadow-sm dark:bg-slate-950 dark:text-white" : "text-slate-500"}`}
          ...
          {error ? <p className="mt-4 rounded-md bg-red-50 px-3 py-2 text-sm font-semibold text-coral dark:bg-red-950/30">{error}</p> : null}

          <Button type="button" className="mt-6 w-full" onClick={submit} disabled={isSubmitting}>
            <UserPlus size={17} />
